...
public class VaultDetailLockedController implements FxController {
	...
	/* This function is called when clicking on the button Unlock.
	 * The unlock process start and one of the two functions is called depending on the Vault type.*/
		
	@FXML
	public void unlock() {
		if (vault.get().getVaultSettings().getVaultsecube()) {
			application.startUnlockWorkflowSecube(vault.get(), Optional.of(mainWindow));
		} else {
